    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http, ObjectMapper objectMapper) throws Exception {
        ...
        return http
            ...
            .formLogin(form->form
                ...
                // 동기방식으로 요청이 들어올 때, 결과를 html로 보여줄때 아래의 코드 사용
                //.defaultSuccessUrl("/index.html") // 로그인 성공 후 보여질 url
                // 비동기 방식으로 요청이 들어올 때는 json으로 결과를 보여줘야하므로,
                // 컨트롤러를 작성하지 않고도 json 결과 전송할 수 있음
                .successHandler((request, response, auth) -> {
                    ...
                    response.getWriter().print(mapper.writeValueAsString(
                            Map.of("result","로그인 성공!","username",auth.getName())
                    ));
                })
                .failureHandler((request, response, exception) -> {
                    ...
                    response.getWriter().print(mapper.writeValueAsString(
                            Map.of("result","로그인 실패", "errorMessage", exception.getMessage())
                    ));
                })
            )
                .build();
